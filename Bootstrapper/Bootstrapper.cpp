	...
	int WinMain(HINSTANCE hInstance, int nShowCmd, Bootstrapper*(*newBootstrapper)(HINSTANCE)) throw()
	{
		...
		int result = 0;
		{
			...
			catch (std::exception& e)
			{
				...
				{
					CString message;
					message.Format(_T("An error occured and Erablox cannot continue.\n\n%S"), e.what());
...
std::shared_ptr<Bootstrapper> Bootstrapper::Create(HINSTANCE hInstance, Bootstrapper*(*newBootstrapper)(HINSTANCE))
{
	...
	if (!result->windowed)
	{
		if (!result->noRun)
		{
			...
			{
				LLOG_ENTRY(result->logger, "No bg update, Erablox App is running");
				return result;
			}
...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode->SetStringValue(_T("Publisher"), _T("Erablox Corporation")), "Failed to set Publisher key");
	throwHRESULT (keyProductCode->SetStringValue(_T("URLInfoAbout"), _T("https://erablox.zip")), "Failed to set URLInfoAbout key");
...
void Bootstrapper::RegisterProtocolHandler(const std::wstring& protocolScheme, const std::wstring& exePath, const std::wstring& installKeyRegPath)
{
	...
	throwHRESULT(key->SetStringValue(_T(""), _T("URL: Erablox Protocol")), format_string("failed to set value for protocol"));
...
bool Bootstrapper::checkBootstrapperVersion()
{
	...
	if (moduleVersionNumber != bootstrapperVersion)
	{
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			try
			{
				message("Getting the latest Erablox...");
			...
			catch (...)
			{
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));		// We could use an "exe" extension, but hiding the type isn't a bad idea?
				{
					...
					std::string eTag;
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-Erablox.exe", installVersion.c_str()), eTag, bootstrapperFile, false, &Bootstrapper::dummyProgress);
				}
			}
...
void Bootstrapper::checkOSPrerequisit()
{
	...
	if (windowed)
		dialog->DisplayError("Erablox requires Microsoft Windows XP SP1 or greater", NULL);
...
void Bootstrapper::checkCPUPrerequisit()
{
    if (!Bootstrapper::hasSse2())
	{
	    ...
	    if (windowed)
		    dialog->DisplayError("Erablox requires SSE2 support", NULL);
...
void Bootstrapper::checkIEPrerequisit()
{
	if (!isIEUpToDate())
	{
		...
		if (windowed)
			dialog->DisplayError("Erablox requires Microsoft Internet Explorer 6.0 or greater", NULL);
...
void Bootstrapper::checkDiskSpace()
{
	...
	{
		dialog->DisplayError("There is not enough room on your disk to install Erablox. Please free up some space and try again.", NULL);
...
void Bootstrapper::run()
{
	...
	try
	{
		...
		if (!IsNetworkAlive(&result) || result == 0)
		{
			...
			{
				CString message = _T("Erablox cannot connect to the Internet\n\nDoes your computer have a working network connection?  Is antivirus software preventing Erablox from accessing the Internet?");
				if (!robloxAppArgs.empty())
				{
					message += _T("\n\nIf you continue Erablox may not work properly.");
		...
		if (!checkBootstrapperVersion())
		{
			...
			if (windowed && isLatestProcess())
			{
				CString message = _T("Cannot connect to the Erablox.\n\nIs antivirus software preventing Erablox from accessing the Internet?");
				if (!robloxAppArgs.empty())
				{
					message += _T("\n\nIf you continue Erablox may not work properly.");
					if (dialog->MessageBox(message, _T("Error"), MB_OKCANCEL | MB_ICONEXCLAMATION) == IDOK)
		...
		if (isFailIfNotUpToDate)
		{
			...
			LOG_ENTRY("Erablox is up to date, returning success");
			goto done;
		}
		...
		bool isNewInstall = false;
		{
			...
			if (!perUser)
			{
				uninstall(true);	// first uninstall per-user Erablox
			}
	...
	{
		exitCode = 1;
		LOG_ENTRY("Erablox is not up to date, returning failure");
	}
	catch (silent_exception&)
	{
...
void Bootstrapper::install()
{
	...
	try
	{
		if (IsWinXP())
		{
			...
			{
				if (windowed)
					dialog->DisplayError("On this machine you must install Erablox using an Administrator account", NULL);
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.erablox.zip/%s/%s/latestProcess", installHost.c_str(), Type().c_str());
